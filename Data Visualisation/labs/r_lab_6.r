...
df_usa_tmp = sqldf('select Gender, Medal, Country from df_usa')
df_usa_tmp
...
df_countires_list
df_countires_list = with(df_usa, df_usa[(Medal == "gold"),])

df_countires_list_tmp = sqldf('select Gender, Medal, Country from df_countires_list')
df_countires_list_tmp


# 4) Who has the most total medals?
...
medal_age = sqldf('select Athlete_Age, Medal from df where Medal != "bronze"')
medal_age = medal_age %>% drop_na()
medal_age
