...
get_plots = function(df, name, columns_to_assess, target){
  ...
  for (col in columns_to_assess){
      bar_plot = ggplot(df, aes_string(x=col,
                  ...
                  ggtitle(paste0("Effect of ", aes_string(x=col), " on customer churn for ", get_var_name(full_customer_dataset))) + 
  ...
  conditional_density_plot = ggplot(df, aes(x=monthlycharges, fill=factor(churn))) + 
           ...
           ggtitle(paste0("Effect of ~monthlycharges on customer churn for ", name)) + 
