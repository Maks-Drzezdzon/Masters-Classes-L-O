...
## 1. H0: There is no difference in social desirability scores for respondents who have chidren and those who do not. HA: There is a difference in social desirability scores for respondents who have chidren and those who do not.
### tmarlow Examine/Assess Normality
```{r tmarlow-normality}

#We will allocate the histogram to a variable to allow use to manipulate it
gg <- ggplot(survey, aes(x=tmarlow))

#Change the label of the x axis
gg <- gg + labs(x="Total Social Desirability")
...
#use stat_function to compute a normalised score for each value of tmarlow
#pass the mean and standard deviation
#use the na.rm parameter to say how missing values are handled
gg <- gg + stat_function(fun=dnorm, color="red",args=list(mean=mean(survey$tmarlow, na.rm=TRUE), sd=sd(survey$tmarlow, na.rm=TRUE)))

#to display the graph request the contents of the variable be shown
gg

#Create a qqplot
qqnorm(survey$tmarlow)
qqline(survey$tmarlow, col=2) #show a line on theplot
...
pastecs::stat.desc(survey$tmarlow, basic=F)
...
tpskew<-semTools::skew(survey$tmarlow)
tpkurt<-semTools::kurtosis(survey$tmarlow)
tpskew[1]/tpskew[2]
...
ztmarlow<- abs(scale(survey$tmarlow))

FSA::perc(as.numeric(ztmarlow), 1.96, "gt")
FSA::perc(as.numeric(ztmarlow), 3.29, "gt")

```
```
Social desirability can be considered to follow a normal distribution (m=`r round(mean(survey$tmarlow, na.rm=TRUE),2)`, sd=`r round(sd(survey$tmarlow, na.rm=TRUE),2)`, n=`r length(survey$tmarlow)-sum(is.na(survey$tmarlow))`, 100% of standardized scores falling within +/- 3.29). Therefore we can use a parametric difference test.
```

### Differences - Parametric Tests
#### Independent t-test
##### Describe social desirability by group (have children v do not have children - variable child)
```{r Describe}
#Get descriptive stastitics by group - output as a matrix
psych::describeBy(survey$tmarlow, survey$child, mat=TRUE)
...
car::leveneTest(tmarlow ~ child, data=survey)
...
stats::t.test(tmarlow~child,var.equal=TRUE,data=survey)
#No statistically significant difference was found

res <- stats::t.test(tmarlow~child,var.equal=TRUE,data=survey)
#Calculate Cohen's d
#artithmetically
...
```
An independent-samples t-test was conducted to compare Social Desirability scores for respondents who have  children and those who do not have children. A statistically significant difference in the scores for Social Desirability was found (M=`r round(mean(survey$tmarlow[survey$child=="YES"], na.rm=TRUE),2)`, SD=`r round(sd(survey$tmarlow[survey$child=="YES"], na.rm=TRUE),2)` for respondents who have children, M=`r round(mean(survey$tmarlow[survey$child=="NO"], na.rm=TRUE),2)`, SD=`r round(mean(survey$tmarlow[survey$child=="NO"], na.rm=TRUE),2)` for respondents who do not have children), (t(`r res$parameter`)= `r round(res$statistic,3)`, p <0.001. Cohen's d also indicated a small effect size (`r effcd`).
...
```
An independent-samples t-test was conducted to compare Social Desirability scores for respondents who have  children and those who do not have children. A statistically significant difference in the scores for Social Desirability was found (M=`r round(mean(survey$tmarlow[survey$child=="YES"], na.rm=TRUE),2)`, SD=`r round(sd(survey$tmarlow[survey$child=="YES"], na.rm=TRUE),2)` for respondents who have children, M=`r round(mean(survey$tmarlow[survey$child=="NO"], na.rm=TRUE),2)`, SD=`r round(mean(survey$tmarlow[survey$child=="NO"], na.rm=TRUE),2)` for respondents who do not have children), (t(`r res$parameter`)= `r round(res$statistic,3)`, p<0.001. A small effect size was also indicated by the eta squared value (`r effes`).
