...
needed_packages <- c("tidyverse", "readxl", "feather", "car", "nortest", "skimr", "nortest", "psych", "ggplot2", "REdaS")                      
...
# to have a better look at the biplot im going to extract the top 4 principal components 
...
The scree plot was slightly ambiguous and showed inflexions that would justify retaining either 0.5 or 2.65/2.75 factors.  
...
The original target variable "sisben" shows little to no correlation with students academic achievement. However after further observation items that are under the sisben umbrella do have an significant effect, such as internet, mobile computer and Tv. Furthermore using a stats ellipse an interesting observation was discovered. Sisben 1 and Sisben 0 are significantly further apart than any other Sisben level, meaning that there is a difference and that this gap is reduced at higher Sisben levels.

Having said that, through other analysis and graphs, it is more likely that the null hypothesis is true. Not having access to the internet, computer or mobile is a reasonable assumption to make that its almost a necessity when assessing exams especially when some students have access to these facilities, meaning that there is little to no statistical significance between increasing government aid/sisben especially after those essential items have been met, this is further supported by the stats ellipse.

An argument could be made/reiterated that increasing levels of sisben do close the gap, but based on the analysis presented above that difference is too small to reject the null hypothesis.
