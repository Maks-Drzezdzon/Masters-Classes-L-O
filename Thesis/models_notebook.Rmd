...
test_func=function(data_split, input_dataset, model){
  train_sample_data = input_dataset[data_split,]
  test_sample_data = input_dataset[-data_split,]
  preds = predict(model, train_sample_data)
  
}

data_split=function(input_ds){
  tmp_data = createDataPartition(y = input_ds$Class, p = 0.8, list=FALSE)
  return(tmp_data)
}
...
size=25
RRF = train(Class ~ ., data=train_sample_data, method="RRF")
...
size = 203
result_rfe_all_features = rfe(x = train_sample_data[,1:411], 
                   y = train_sample_data$Class, 
...
# LASSO #
#########
lasso_cv = cv.glmnet(as.matrix(train_sample_data[,1:410]), as.double(train_sample_data$Class), family='binomial', alpha=1, parallel=TRUE, standardize=TRUE, type.measure='mse', keep=T)
...
Boruta_features_data_without_tentative = subset(train_sample_data, select = rownames(imps2_without_tentative))
