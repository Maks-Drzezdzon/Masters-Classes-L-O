...
RRF = train(Class ~ ., data=data_with_dummy_var, method="RRF")
RRF_features = varImp(RRF, scale=F)
RRF_features
plot(RRF_features, top = 20, main='Variable Importance')

# ignore below this 
#random_forest = randomForest(Class ~ ., data=tmp_data, 
#                             ntree = 2000, mtry = 100, 
#                             importance=TRUE, proximity=TRUE)  

# drop everything below dummy_Var
#importance_df = data.frame(random_forest$importance)
#by_ill_acc_gain = importance_df[order(importance_df$X0, importance_df$MeanDecreaseAccuracy),]
#by_ill = importance_df[order(importance_df$X1),]
#by_acc_gain = importance_df[order(importance_df$MeanDecreaseAccuracy),]
...
tmp = subset(train_data, select= -c(Class))


result_rfe1 = rfe(x = tmp, 
                   y = train_data$Class, 
                   sizes = c(20:60), # best 20-60 features
                   rfeControl = control)
...
arimp_data = data.frame(feature = row.names(varImp(result_rfe1))[1:60],
                          importance = varImp(result_rfe1)[1:60, 1])
...
# TODO DONE
lasso_cv = cv.glmnet(as.matrix(tmp_data[,1:410]), as.double(tmp_data$Class), family='binomial', alpha=1, parallel=TRUE, standardize=TRUE, type.measure='auc')
...
boruta_output = Boruta(Class ~ ., data=na.omit(tmp_data), doTrace=0)  
...
boruta_signif_2 = getSelectedAttributes(roughFixMod)
print(boruta_signif_2)
...
rPartMod = train(Class ~ ., data=tmp_data, method="rpart")
...
lm_base = lm(Class ~ 1 , data=data_with_dummy_var[,1:411])  

# Step 2: Full model with all predictors
lm_full = lm(Class ~ . , data=data_with_dummy_var[,1:411]) 
