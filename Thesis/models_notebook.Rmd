...
fplasso <- glmnet(tmp_data[,1:410], tmp_data$Class, family = "multinomial", intercept = F, alpha=1)
fplasso
...
set.seed(100)
cv.lasso <- cv.glmnet(as.matrix(tmp_data[,1:410]), as.double(tmp_data$Class), family='binomial', alpha=1, parallel=TRUE, standardize=TRUE, type.measure='auc')
...
rrfMod = train(Class ~ ., data=tmp_data, method="RRF")
...
base.mod <- lm(Class ~ 1 , data=tmp_data)  

# Step 2: Full model with all predictors
all.mod <- lm(Class ~ . , data= tmp_data) 
