...
test_sample_data$Class = as.matrix(test_sample_data$Class)
train_sample_data = data.frame(lapply(train_sample_data, as.numeric))
test_sample_data = as.matrix(data.frame(lapply(test_sample_data, as.numeric)))
x = train_sample_data


# fit a linear DWD
kern = vanilladot()
dwd_linear = kerndwd(x, f, kern)
dwd_linear
preds = predict(dwd_linear, kern, as.matrix(x), test_sample_data)
preds

auc = Metrics::auc(tmp_test_sample$Class, preds)
cf = confusionMatrix(preds, as.factor(tmp_test_sample$Class), mode="everything", positive = "1")
